...
function MyAppointmentsView({ onClose, appointments, setAppointments, addNotification }) {
  ...
  // Add this to the MyAppointmentsView component, right after the useEffect for incoming calls (around line 600)
  // This will automatically end the first call after 15 seconds

  // In the MyAppointmentsView component, replace this useEffect:
  useEffect(() => {
...
function SCAD() {
  ...
  // Add a new state for workshops and workshop management modal
  // Add this after the other state declarations (around line 1000)
  ...
  // Add this to the handleActionClick function in the SCAD component
  // Find the handleActionClick function and modify it to include the Workshop action
  const handleActionClick = (actionName) => {
    ...
    else if (actionName === "Workshop") setShowWorkshops(true)
    else {
  ...
  // Add these workshop management functions after the other handler functions
  ...
  const handleDeleteWorkshop = (id) => {
    if (confirm("Are you sure you want to delete this workshop?")) {
      ...
      addNotification("Workshop deleted successfully", "success")
    }
  }
  ...
  const handleWorkshopSubmit = (e) => {
    ...
    if (selectedWorkshop) {
      ...
      addNotification("Workshop updated successfully", "success")
    } else {
      // Create new workshop
      setWorkshops([...workshops, workshopFormData])
      addNotification("New workshop created successfully", "success")
    }
  ...
  const closeWorkshopsSection = () => {
    setShowWorkshops(false)
    setSelectedWorkshop(null)
    setIsEditingWorkshop(false)
  }

  // Add the WorkshopsView component before the return statement of the SCAD component
  function WorkshopsView({
    ...
    const renderWorkshopForm = () => (
      <div className="workshop-form-container">
        <div className="workshop-form-header">
          ...
          <button className="modal-close-button" onClick={() => onSubmit({ preventDefault: () => {} })}>
            <X size={20} />
          </button>
        </div>
        <form onSubmit={onSubmit} className="workshop-form">
          ...
          <div className="agenda-section">
              ...
              <div key={index} className="agenda-item">
                <div className="form-group agenda-input">
                  ...
                  <input
                    ...
                    id={`agenda-${index}`}
                    value={item}
                    onChange={(e) => onAgendaItemChange(index, e.target.value)}
                    required
                  />
                </div>
                <button
            ...
            <button type="button" className="add-agenda-btn" onClick={onAddAgendaItem}>
              + Add Agenda Item
            </button>
          </div>
      ...
      <div className="workshop-details">
        <div className="workshop-details-header">
          ...
          <div className="workshop-actions">
            <button className="edit-workshop-btn" onClick={() => onEdit(selectedWorkshop)}>
              <Edit size={16} /> Edit
            </button>
            <button className="delete-workshop-btn" onClick={() => onDelete(selectedWorkshop.id)}>
              <X size={16} /> Delete
            </button>
            <button className="modal-close-button" onClick={() => onView(null)}>
              <X size={20} />
            </button>
          </div>
        </div>
    ...
    const renderWorkshopsList = () => (
      <div className="workshops-list-container">
        <div className="workshops-header">
          ...
          <button className="create-workshop-btn" onClick={onCreate}>
        ...
        {upcomingWorkshops.length > 0 ? (
          <div className="workshops-list">
            {upcomingWorkshops.map((workshop) => (
              <div key={workshop.id} className="workshop-card" onClick={() => onView(workshop)}>
    ...
    return (
      <div className="workshops-section-overlay">
        {isEditing ? renderWorkshopForm() : selectedWorkshop ? renderWorkshopDetails() : renderWorkshopsList()}
      </div>
    )
  }
    ...
    <div className="scad-container">
      ...
      <main className="scad-main">
        ...
        {showWorkshops && (
          <WorkshopsView
            workshops={workshops}
            onClose={closeWorkshopsSection}
            onEdit={handleEditWorkshop}
            onCreate={handleCreateWorkshop}
            onDelete={handleDeleteWorkshop}
            onView={handleViewWorkshopDetails}
            selectedWorkshop={selectedWorkshop}
            isEditing={isEditingWorkshop}
            formData={workshopFormData}
            onFormChange={handleWorkshopFormChange}
